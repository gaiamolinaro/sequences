...
from pseudoRCtrl_tf_to_csv import *

# Task parameters
bandit_set = 8  # number of bandits to use # Sam had 16
all_goal_sets = [[5, 17], [25, 26], [24, 30], [56, 59], [76, 77]] # fractals to use in the learning phase, includes goal and non-goal
prop_new_vs_old = 0.5  # proportion of new vs old images shown
tot_fract_set = 2  # total number of fractals available
...
stim_vec = np.empty((num_subs, tot_fract_set))  # initialize

for i in range(0, num_subs, 2):  # floor division a//b = int(a/b)
    # first pair of subjects have same images for the task but they are opposite in value (goal vs no goal)
    learning_fract_set = all_goal_sets[np.random.randint(0, len(all_goal_sets))]
    np.random.shuffle(learning_fract_set)  # shuffle
    stim_vec[i] = learning_fract_set 
    inverted = np.roll(stim_vec[i], len(stim_vec[i])//2)  # new (shifted/flipped) vector
    stim_vec[i+1] = inverted  # counterbalanced subject group with opposite goal vs nongoal images
# Main loop
for si in range(num_subs):
    ...
    for j in blocks:
        ...
        for t in range(learning_trials_per_block):
            ...
            #'''
                tf["trial_type"][j, t] = 1
                tf["goal_stim"][j, t] = stim_vec[si, 0]  # snag novel goal stim
                tf["nongoal_stim"][j, t] = stim_vec[si, 1]  # snag novel non - goal stim 
    ...
    #'''
    # Produce a csv file
    to_dir = "C:/Gaia/CCN Lab/pseudoR Project/pseudoR Codes/pseudoRCtrl/sequences/"
    for j in blocks:
        tf_to_task_seq(tf, si, j, to_dir)
    #'''