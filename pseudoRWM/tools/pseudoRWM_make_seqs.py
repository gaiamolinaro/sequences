...
if use_matlab:
    ...
    m.addpath("C:/Gaia/CCN Lab/pseudoR Project/pseudoR Codes/pseudoRWM/", nargout=0)
else:
...
for si in range(num_conditions):
    # blocks = np.array([2, 3, 4, 5, 6, 3, 5, 2, 3, 4, 5, 6]) # each element represents a block's set size (change if needed)
    # blocks = np.array([6, 6, 6, 6, 6, 6, 6]) # 7 blocks: 1 goal + 3*type
    blocks = np.array([6, 6, 6, 6, 6, 6, 6]) # 6 blocks: 3*type
    # trial_types = np.array([0, 1, 0, 1, 0, 1, 0, 1]) # 1 = points, 0 = goals
    # trial_types = [0] # first trial is always goal
    trial_types = []
    for tp in range(3):
        for tt in np.random.permutation(2):
            trial_types.append(tt)
    trial_types = np.array(trial_types)
    # block_idx = np.hstack((np.array([0]), (np.repeat(np.arange(1, len(blocks), 2), 2) + trial_types[1:])))
    block_idx = np.hstack(((np.repeat(np.arange(1, len(blocks), 2), 2) + trial_types)))
    blocks = blocks[block_idx]

    max_ns = np.max(blocks)
    # p = np.random.permutation(4) # permutate ns 3,4,5,6 (change int arg if needed)
    # blocks = np.hstack((blocks[0], blocks[p+1], blocks[6:9], blocks[p+8])) # rearrange fixed/permutated blocks - change if needed
    # p = np.random.permutation(len(blocks))
    # blocks = blocks[p]
    # trial_types = trial_types[p]
